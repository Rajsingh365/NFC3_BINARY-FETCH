...
const FreeFireInfo = ({ maxRate, equippedSkills }) => {
  // console.log(equippedSkills);
  
  const data = {
    labels: equippedSkills.map((skill, index) =>  "Month "+(index+1)),
    datasets: [
      {
        label: "Performance",
        data: equippedSkills,
  ...
  const maxRank = maxRate;
  return (
    <motion.div
      initial={{ opacity: 0 }}
      whileInView={{ opacity: 1 }}
      transition={{ duration: 1, ease: easeInOut }}
      className="bg-[#21222D] py-10 rounded-lg w-[60rem] h-[50rem]"
    >
      <h2 className="text-3xl font-semibold ml-20 pb-5">FreeFire Analytics</h2>
      <motion.div className=" gap-x-5 flex items-center justify-center p-5 text-3xl text-center ">
        ...
        >
          <div className="stat place-items-center">
            ...
            <div className="stat-title">Max Rank</div>
            <div className="stat-value text-[#CB964D]">{maxRank}</div>
          </div>
        </motion.div>
